...
module.exports = function(app) {
  ...
  app.post('/notes', function(req, res) {
    models.sequelize.sync().on('success', function() {
      Note.create({UserId: oyinda, CategoryId: req.param('CategoryId'), link: req.param('link'), title: req.param('title'), content: req.param('content'), description: req.param('description'), icon: req.param('icon')}).success(function(notes) {
        res.json(notes);
      })
    });
  });
  ...
  app.put('/notes', function(req, res) {
    ...
    models.sequelize.sync().on('success', function() {
      Note.find({where: {id: noteId}, attributes: noteSafeParams, include: [Category]}).success(function(note) {
        ...
        // Return an 401 aunauthorized if a user tries to editor another user's note 'req.user.id' 
        if(!req.user ||oyinda !== note.values.UserId) {
          res.status(401);
